...
model_no = 0 # to be passed in as a parameter
...
class OptionDefClass:
    def __init__(self, en):
        self.con_id = en.con_id
        self.strike = en.strike
        self.expiry = en.expiry
        self.expiry_do = en.expiry_do
    ...
    for open_stock in pdStocks.itertuples():
        ...
        for optionDef in pdOptions.itertuples():
            ...
            if ml_model.model_logic.open_position(model_no, open_stock.bid, open_stock.ask, open_option.bid,
                ...
                x = OptionDefClass(optionDef)
                fut.append(executor.submit(eval_execute_trade, ctr, maxStockIndex, model_no, npStocks, oexp, open_iv, open_option, open_stock,
                                       open_theta, open_tv, x, pdOptionQuotes_by_timeContractNo))
                # eval_execute_trade(ctr, maxStockIndex, model_no, npStocks, oexp, open_iv,
                #                   open_option, open_stock, open_theta, open_tv, optionDef,
                #                   pdOptionQuotes_by_timeContractNo, rows)

    while True:
        if len(fut) == 0: break
        for f in fut:
            if f.done():
                print('Resulte: ', f.result())
...
def eval_execute_trade(ctr, maxStockIndex, model_no, npStocks, oexp, open_iv, open_option, open_stock, open_theta,
                       open_tv, optionDef, pdOptionQuotes_by_timeContractNo):
    sold = False
